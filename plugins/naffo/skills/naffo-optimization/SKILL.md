...
For a single product, follow these steps in order. Never skip — each step guards
the next, and every tool below is live on naffo.tech.

If a deployment ever answers `missing` for one of these (an older self-hosted
build), fall back as shown rather than fabricating the step's output:

| Step | Tool | Fallback if a deployment lacks it |
|---|---|---|
| 2 | `naffo_get_demand_features` | Read the pattern off `naffo_get_demand_series`; no `confidence_boost` |
| 3 | `naffo_get_demand_trend` | Compare period means from `naffo_get_demand_series` |
| 4 | `naffo_detect_changepoint` | Ask: "Have you lost a customer or discontinued a line?" Apply no dampening without an answer |
| 6 | `naffo_aggregate_forecast_range` | Present the naive `p10_total`..`p90_total` and say the range overstates uncertainty |
| 7 | `naffo_newsvendor_order_qty` | Cover to `p50_total − stock_on_hand − pending_supply` |
| 8 | `naffo_harden_order_decision` | Apply MOQ and lead time by hand from `naffo_get_product_forecast_context` |
| 9 | `naffo_forecast_scenarios` | Build LOW/EXPECTED/HIGH from p10/p50/p90 minus stock |
