...
skill. Every tool in the chain is live on naffo.tech — do not skip steps, and do
not fabricate a step you couldn't run.
...
4. Demand character — `naffo_get_demand_features` (pattern, YoY growth, momentum).
5. Trend direction — `naffo_get_demand_trend({ periods: 6 })`.
6. Structural drop guard — `naffo_detect_changepoint`. Mandatory when trend is
...
7. Probabilistic forecast — `naffo_forecast_demand` with the user's context answers
   ...
   `confidence_tier`.
8. Correct the range — `naffo_aggregate_forecast_range` using the daily series from
...
10. Cost-optimal quantity — `naffo_newsvendor_order_qty` (needs unit_cost / sell_price;
    otherwise cite `basis: "fallback_no_costs"`).
11. Business guardrails — `naffo_harden_order_decision`. Always present
    `final_order_qty`, plus `decision_status`, `urgency`, and every guardrail flag.
12. Three scenarios — `naffo_forecast_scenarios`. Always show LOW / EXPECTED / HIGH
